...
class ChatScreenState extends State<ChatPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    socketIO.subscribe('kick_message1', (jsonData) {
      ...
      hasData = true;
      KickMessage message = KickMessage(
          data: data,
          animationController: AnimationController(
      ...
      for (int i = 0; i < load.length; i++) {
        if (load[i]['message'] == null) {
          if (load[i]['type'] == 'enter')
            _message.add(EnterMessage(
              data: load[i],
              animationController: AnimationController(
                  vsync: this, duration: Duration(milliseconds: 5)),
            ));
          else if (load[i]['type'] == 'kick')
            _message.add(KickMessage(
              data: load[i],
              animationController: AnimationController(
                  vsync: this, duration: Duration(milliseconds: 5)),
            ));
          else
            _message.add(LeaveMessage(
              data: load[i],
              animationController: AnimationController(
                  vsync: this, duration: Duration(milliseconds: 5)),
            ));
        } else if (load[i]['id'] == shared.getString('id')) {
          _message.add(ChatMessageS(
              data: load[i],
              animationController: AnimationController(
                  vsync: this, duration: Duration(milliseconds: 5))));
        } else {
          _message.add(ChatMessageR(
              data: load[i],
              animationController: AnimationController(
                  vsync: this, duration: Duration(milliseconds: 5))));
        }
                              ...
                              children: <Widget>[
                                  ...
                                  child: Column(
                                    ...
                                    children: <Widget>[
                                      ...
                                      Text(
                                        ...
                                        style: mbtiList[i] != null
                                            ? TextStyle(
                                                fontSize: 8,
                                                color: mbti_color[mbtiList[i]],
                                                fontWeight: FontWeight.bold)
                                            : TextStyle(
                                                fontSize: 8,
                                                color: Colors.black,
                                ...
                                Container(
                                  ...
                                  child: IconButton(
                                    ...
                                    onPressed: () async {
                                      comments = await apiService
                                          .show_comment(idList[i]);
                                      showDialog(
                                        context: context,
                                        builder: (BuildContext context) =>
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text(
                    ...
                    style: bossmbti != null
                        ? TextStyle(fontSize: 10, color: mbti_color[bossmbti])
      ...
      body: Container(
        decoration: BoxDecoration(
            image: DecorationImage(
                ...
                image: AssetImage(
                    travel_spots[int.parse(widget.category) - 1]['img']))),
        child: Column(
          children: <Widget>[
            // 리스트뷰를 Flexible로 추가.
            Flexible(
              ...
              itemBuilder: (_, i) {
                ...
                if (comments[i]['img'] == 'x') {
                  if (comments[i]['mbti'] != null) {
                    c = AssetImage(
                        'assets/mbti/' + comments[i]['mbti'] + '.png');
                  } else {
                    c = AssetImage('assets/person.png');
                  }
                } else {
                  c = CachedNetworkImageProvider(
                      'http://$myIP:3001/${comments[i]['img']}');
                }
                return Container(
                    ...
                    children: <Widget>[
                      Column(
                        ...
                        children: <Widget>[
                          ...
                          Text(
                            comments[i]['mbti'] != null
                                ? comments[i]['mbti']
                                : 'x',
                            style: comments[i]['mbti'] != null
                                ? TextStyle(
                                    fontSize: 10,
                                    color: mbti_color[comments[i]['mbti']])
                                : TextStyle(fontSize: 10, color: Colors.black),
                          )
                        ],
                      ),
  ...
  Widget buildEvaluateDialog(BuildContext context, int i) {
    ...
    return new AlertDialog(
      ...
      actions: <Widget>[
        new FlatButton(
            onPressed: () async {
              Map<String, dynamic> response = await apiService.comments(
                  shared.getString('id'),
                  shared.getString('username'),
                  shared.getString('mbti'),
                  shared.getString('img'),
          ...
          Column(
            ...
            children: <Widget>[
              Text(
                ...
                style: data['mbti'] != null
                    ? TextStyle(
                        fontSize: 10,
                        color: mbti_color[data['mbti']],
                        fontWeight: FontWeight.bold)
                    : TextStyle(
                        fontSize: 10,
                        color: Colors.black,
      ...
      child: Row(
        ...
        children: <Widget>[
          Column(
            ...
            children: <Widget>[
              Text(
                  shared.getString('mbti') != null
                      ? shared.getString('mbti')
                      : 'x',
                  style: shared.getString('mbti') != null
                      ? TextStyle(
                          fontSize: 10,
                          color: mbti_color[shared.getString('mbti')],
                          fontWeight: FontWeight.bold)
                      : TextStyle(
                          fontSize: 10,
                          color: Colors.black,
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        ...
                        style: data['mbti'] != null
                            ? TextStyle(
                                fontSize: 10, color: mbti_color[data['mbti']])
                            : TextStyle(fontSize: 10, color: Colors.black),
                      )
                    ],
                  ),
